...
JNIEXPORT jobject JNICALL Java_PacWarGuts_fastDuel(JNIEnv* env, jclass clazz, jstring g1, jstring g2, jint maxRounds) {
	int rounds,c1,c2;
	PacGene pg1,pg2;
	jclass class_DuelResults;
	jmethodID mid_DuelResults_constructor;
	jobject duelResults;
	const char* str;

	str=(*env)->GetStringUTFChars(env,g1,0);
	memcpy(&pg1,str,50);
	(*env)->ReleaseStringUTFChars(env,g1,str);
	str=(*env)->GetStringUTFChars(env,g2,0);
	memcpy(&pg2,str,50);
	(*env)->ReleaseStringUTFChars(env,g2,str);
	rounds=maxRounds;
	FastDuel(&pg1,&pg2,&rounds,&c1,&c2);

	class_DuelResults=(*env)->FindClass(env,"DuelResults");
	mid_DuelResults_constructor=(*env)->GetMethodID(env,class_DuelResults,"<init>","(III)V");
	duelResults=(*env)->NewObject(env,class_DuelResults,mid_DuelResults_constructor,rounds,c1,c2);
